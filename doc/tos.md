# Terms of Service — TeamsVote

**Last Updated:** _2026-01-26_

These Terms of Service (“Terms”) govern the use of **TeamsVote** (“Service”), a Microsoft Teams plugin made available at <https://github.com/Marvin-Brouwer/TeamsVote>, provided by the developer (“we”, “us”, “our”). By using the Service within Microsoft Teams, you (“you”, “user”) agree to be bound by these Terms.

If you do not agree to these Terms, you may not use the Service.

---

## 1. Description of the Service

TeamsVote is a plugin for Microsoft Teams that enables users to conduct lightweight vote or polling sessions within Teams channels, chats, or meetings. The Service operates fully within the Microsoft Teams environment and does not function as a standalone application.

---

## 2. Eligibility

You may use the Service only if you have access to a valid Microsoft Teams account and are authorized by your organization or administrator to install and use plugins.

---

## 3. Relationship with Microsoft Teams

TeamsVote operates as an integration within Microsoft Teams. Usage of TeamsVote is subject to and dependent on Microsoft’s platform policies, including but not limited to:

- Microsoft Teams Terms of Service
- Microsoft 365 policies
- Relevant organizational policies

Users must comply with Microsoft’s applicable terms and are solely responsible for ensuring compliance. We are not affiliated with or endorsed by Microsoft.

---

## 4. User Information and Data Handling

TeamsVote does not create or manage its own user accounts. The Service relies exclusively on identity and access provided by Microsoft Teams.

We do **not** store personal data beyond what is temporarily required for the active session. Specifically:

- Data is processed **only in-memory**
- Data is retained **only for the duration of the session**
- Data is **not persisted**, logged, exported, or transmitted to external services

When the session ends, all session data is discarded.

User data remains under the control of Microsoft and/or the organization administering Microsoft Teams.

---

## 5. Security & Privacy

We take reasonable measures to ensure the Service operates securely within the Microsoft Teams ecosystem. However, we cannot guarantee that:

- the Service is free from vulnerabilities,
- the platform will not experience outages or disruptions, or
- unauthorized access will never occur.

Your organization is responsible for managing access and permissions through Microsoft Teams.

---

## 6. Payments

TeamsVote is provided **free of charge**. The Service does not process payments, financial information, or billing.

---

## 7. Acceptable Use

Users agree not to:

- use the Service for unlawful activities,
- interfere with the functionality of Microsoft Teams,
- attempt to reverse-engineer or modify the Service,
- use the Service to collect, monitor, or profile individuals, or
- use the Service in violation of Microsoft’s policies or applicable law.

---

## 8. Service Availability & Modifications

We may modify, suspend, or discontinue the Service at any time, with or without notice. We are not liable for any resulting impact, including organizational workflow interruptions.

---

## 9. Disclaimer of Warranties

The Service is provided **“as-is”** and **“as available”**, without warranties of any kind, express or implied. We disclaim all warranties including:

- merchantability,
- fitness for a particular purpose,
- non-infringement, and
- availability or uptime guarantees.

---

## 10. Limitation of Liability

To the maximum extent permitted by law, in no event will we be liable for any indirect, incidental, consequential, special, or punitive damages arising out of the use of the Service, including but not limited to:

- loss of data,
- interruption of business,
- compliance failures, or
- security incidents originating outside our control.

---

## 11. Governing Law

These Terms shall be governed by and construed in accordance with the laws of **Europe**, without regard to conflict of law principles. If your organization imposes jurisdictional requirements, those may apply instead.

---

## 12. Changes to These Terms

We may update these Terms from time to time. Continued use of the Service after changes become effective constitutes acceptance of the updated Terms.

---

## 13. Contact

If you have questions about these Terms, please refer to the project repository at:  
<https://github.com/Marvin-Brouwer/TeamsVote/issues>
